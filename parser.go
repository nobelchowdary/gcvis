...
const (
	...
	GCRegexpGo20 = `gc #?\d+ @(?P<ElapsedTime>[\d.]+)s \d+%: (?P<STWSclock>[^+]+)\+(?P<MASclock>[^+]+)\+(?P<STWMclock>[^+]+) ms clock, (?P<STWScpu>[^+]+)\+(?P<MASAssistcpu>[^+]+)/(?P<MASBGcpu>[^+]+)/(?P<MASIdlecpu>[^+]+)\+(?P<STWMcpu>[^+]+) ms cpu, \d+->\d+->\d+ MB, (?P<Heap1>\d+) MB goal, (?P<Stacks>\d+) MB stacks, (?P<Globals>\d+) MB globals, \d+ P`
...
var (
	...
	gcrego20 = regexp.MustCompile(GCRegexpGo20)
...
func (p *Parser) Run() {
	...
	for sc.Scan() {
		...
		if result := gcrego20.FindStringSubmatch(line); result != nil {
			//fmt.Printf("%v +++++++++++++++", result)
			p.GcChan <- parseGCTrace(gcrego20, result)
			continue
		}
...
func parseGCTrace(gcre *regexp.Regexp, matches []string) *gctrace {
	...
	return &gctrace{
		Globals:      silentParseInt(matchMap["Globals"]),
		Stacks:       silentParseInt(matchMap["Stacks"]),
